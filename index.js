...
function addSpace(spaceKey) {
    if (spaceKey == userInfo[currentRound].gameString[currentIndex]) {
        let val = document.getElementById('inputField').value.slice(0, -1); // creates value without the space at the end
...
function colorText(key) {
    ...
    //coloredLetter.setAttribute('class', white);
...
function modalInfo() {
    document.getElementById('modalText').innerHTML = '<h4>Round ' + currentRound + ' completed!</h4><br><p>Errors: ' + userInfo[currentRound].errors;
...
function startNewRound() {
    ...
    //document.getElementById('mainP').setAttribute('class', 'senClass');
    //document.getElementById('mainBox').setAttribute('class', 'senClass');
    document.getElementById('inputField').value = '';
    ...
    hideElements([document.getElementById('startBtn'), document.getElementById('modalPopup')]);
}
